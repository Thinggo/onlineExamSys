...
public class QuestionDao {
	private String colums = " id " + ",title" + " ,courseId" + ",dbo.getcourseNameByid(courseId) courseName"
			+ " ,courseunitId" + " ,dbo.getUnitNameByid(courseunitId) unitName" + " ,typeId"
			+ " ,dbo.getQuestionTypeByid(typeId) questionType" + " ,deleteFlag" + "  ,answer" + " ,defficulty"
			+ " ,explanin" + " ,teacherId," + "  dbo.getTeacherByid(teacherId) teacher" + ", createDate";
	 ...
	 * @teacherId int,
	 * 
	 * @questionOption varchar(500) ----asscii 9 和 11 作为分离符 --选项之间用 11
	 * --选项内容与是否正确用 9
	 */
	...
	public void insert(Question q, List<QuestionOption> qo) throws SQLException {
		Connection conn = DbConn.getDbConn();
		java.sql.CallableStatement cstm = conn.prepareCall("{call insertQuestion(?,?,?,?,?,?,?,?,?)}");
		...
		for (int i = 0; i < qo.size(); i++) {
			...
			optionStr = optionStr + name + String.valueOf((char) 9) + isRight + String.valueOf((char) 11);
	...
	public void insert(Question q) throws SQLException {
		Connection conn = DbConn.getDbConn();
		String sql = "INSERT INTO question(title ,courseId" + " ,courseunitId ,typeId ,answer ,defficulty"
				+ " ,explanin ,teacherId )" + "  VALUES(?,?,?,?,?,?,?,?) ";
		if (conn != null) {
	...
	public void update(Question q) throws SQLException {
		...
		String sql = "update question " + "  set title=?,courseunitId=?," + "  answer=?,defficulty=?," + "  explanin=?"
				+ "  where id=?";
	 ...
	 */
	public PagerModel<Question> list(int uid, String order, int pageSize, int pageIndex, String where)
			throws SQLException {
		...
		pstm = conn.prepareCall("{call sp_Pager('question','" + colums + " ',?,?,?,?,?)}");
	...
	private List<QuestionOption> findOptionByQuestionId(int questionId) throws SQLException {
		...
		Connection conn = DbConn.getDbConn();
		String sql = "select id,name,seqNo,isRight " + " from questionOption " + " where questionId=?";
		if (conn != null) {
